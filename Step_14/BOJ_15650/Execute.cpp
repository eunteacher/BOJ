...
/*
 * DFS() 최적화
 * 재귀 호출에서 원소의 이전 값들을 방문하지 않도록 했다.
 * 시작값을 매개변수로 주어 더 작은 값만 계산
 */
void DFS2(vector<int> seq, vector<bool> visited, int n, int m, int start, int startNum)
{
	// 출력
	if (start == m)
	{
	...
	for (int i = startNum; i < n + 1; i++)
	{
		if (visited[i] == false)
		{
			visited[i] = true; // 상태 변화
			seq[start] = i;
			DFS2(seq, visited, n, m, start + 1, i + 1); // 재귀 호출
			visited[i] = false; // 상태 복구
		}
	}
}
